...
class Config:
    ...
    def __load_pillar_config(project_config: TOMLSettings) -> PyObject:
        ...
        pillar_config = Path(project_config.config).expanduser()
        ...
        return getattr(module, project_config.pillar)

    def __merge_settings(self, loaded_settings: Dict[str, Any]):
        """
        ...
        prefix = self.settings.Config.env_prefix

        for setting, value in loaded_settings.items():
            if f"{prefix}{setting}" not in env_vars:
    ...
    def load(self, file: Union[str, Path]):
        ...
        logging.debug('Successfully loaded "%s"', project_config.config.absolute())
