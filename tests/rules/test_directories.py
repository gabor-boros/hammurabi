...
class ExampleSingleDirectoryRule(SingleDirectoryRule):
    def task(self, param: Path) -> Path:
        return param


def test_single_directory_rule():
    """
    Since SingleDirectoryRule's base classes are tested from
    ...
    rule = ExampleSingleDirectoryRule(
        name="Single directory base rule", path=expected_param
    )
...
def test_exists():
    mocked_path = Mock()
    rule = DirectoryExists(name="Test dir exists")

    result = rule.task(mocked_path)
...
def test_not_exists_no_remove(mocked_shutil):
    ...
    rule = DirectoryNotExists(name="Test dir not exists")
...
def test_emptied_files_or_symlinks(mocked_os, mocked_shutil, mocked_path):
    fake_path = Mock()
    rule = DirectoryEmptied(name="Test dir emptied")
